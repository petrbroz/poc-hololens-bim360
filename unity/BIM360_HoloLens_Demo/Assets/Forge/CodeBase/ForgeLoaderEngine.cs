...
namespace Autodesk.Forge.ARKit {
	...
	public class ForgeLoaderEngine : IForgeLoaderInterface {
		...
		protected void requestMesh (object sender, IRequestInterface args) {
			...
			reqObj.uri = new System.Uri (ForgeLoaderConstants._endpoint + URN + "/mesh/" + reqObj.fragId.Item1 + "/" + reqObj.fragId.Item2);
		...
		public static GameObject SetupForSceneOrientationAndUnits (GameObject sceneRoot, JSONNode metadata) {
			...
			Vector3 pivot = /* PetrBroz disabling ref point: pt.AsArray.Count == 3 ? new Vector3 (pt [0].AsFloat, pt [1].AsFloat, pt [2].AsFloat) : */ Vector3.zero;
			...
			float unitsConvert = 0.1f;//ForgeLoaderEngine.convertToMeter (units);
